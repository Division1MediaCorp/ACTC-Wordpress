<?php
add_action('widgets_init', 'citygov_featured_widget');

function citygov_featured_widget()
{
	register_widget('citygov_featured_widget');
}

class citygov_featured_widget extends WP_Widget {
	
	public function __construct() {
		$widget_ops = array('classname' => 'citygov_featured_widget', 'description' => esc_html__('Featured posts widget.','citygov'));
       	parent::__construct(false, esc_html__('Themnific: Featured','citygov'),$widget_ops);  
	}
	...
	function widget($args, $instance)
	{
		...
		?>
        	...
        	<?php if ( $title == "") {} else { ?>
				...
				<h2 class="widget dekoline"><span><a href="<?php echo get_category_link($categories); ?>"><?php echo esc_attr($title); ?></a></span></h2>
