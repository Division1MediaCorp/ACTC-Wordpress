<?php
add_action('widgets_init', 'citygov_blog_list');

function citygov_blog_list()
{
	register_widget('citygov_blog_list');
}

class citygov_blog_list extends WP_Widget {
	
	public function __construct() {
		$widget_ops = array('classname' => 'citygov_blog_list', 'description' => esc_html__('!!Posts block for Elementor only!!','citygov'));
       	parent::__construct(false, esc_html__('Themnific: Blog List','citygov'),$widget_ops);  
	}
	...
	function widget($args, $instance)
	{
		...
		?>
        	...
        	<?php if ( $title == "") {} else { ?>
				...
				<h2 class="widget content_widget"><span><a href="<?php echo get_category_link($categories); ?>"><?php echo esc_attr($title); ?></a></span></h2>
			...
			<?php  while($recent_posts->have_posts()): $recent_posts->the_post();if(has_post_format('aside')){ } elseif(has_post_format('quote')){ }else { ?>
                            ...
                            <?php if(has_post_format('aside')){} else {
								get_template_part('/post-types/post-front');
                            }?>
	...
	{
		$defaults = array('title' => 'Recent Posts', 'post_type' => 'all', 'categories' => 'all', 'posts' => 4, 'offset_posts' => '');
